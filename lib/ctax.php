...
// hook into the init action and call create_producten_tax when it fires
add_action( 'init', 'create_producten_tax', 0 );

function create_producten_tax() {
	...
	$args = array(
		...
		'show_admin_column' => true,
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'producten-categorie' ),
	);

	register_taxonomy('producten-categorie', array('producten'), $args);

}


// hook into the init action and call create_book_taxonomies when it fires
add_action( 'init', 'create_profonts_tax', 0 );

function create_profonts_tax() {
	...
	$args = array(
		...
		'show_admin_column' => true,
		'query_var'         => true,
		'rewrite'           => array( 'slug' => 'fonts-categorie' ),
	);

  register_taxonomy('fonts-categorie', array('pro_fonts'), $args);
